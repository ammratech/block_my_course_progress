...
class block_my_course_progress extends block_base {
    ...
    public function get_content() {
		...
		$PAGE->requires->js_call_amd('block_my_course_progress/block', 'init');
    ...
    data-name="'.strtolower($course->fullname).'">
                    <th>' . get_string('course', 'block_my_course_progress') . '</th>
					<th>' . get_string('category', 'block_my_course_progress') . '</th>
                    <th>' . get_string('status', 'block_my_course_progress') . '</th>
					<th align="center">' . get_string('module_count', 'block_my_course_progress') . '</th>
                    <th>' . get_string('progress', 'block_my_course_progress') . '</th>
                </tr>
            </thead><tbody>';

        foreach ($courses as $course) {

            $status = get_string('enrolled', 'block_my_course_progress');
            $progress = 0;
            ...
            //if ($course->enablecompletion) {
                ...
                if ($progress == 0 && $vLastAccess == 0) {
                    $status = get_string('notstarted', 'block_my_course_progress');
                } else if ($progress < 100 and $vLastAccess > 0) {
                    $status = get_string('incomplete', 'block_my_course_progress');
                    $class = 'status-incomplete';
                } else {
                    $status = get_string('completed', 'block_my_course_progress');
